//
//  LPShortFreightViewController.m
//  LePin-Ent
//
...
@interface LPShortFreightViewController ()<UITableViewDelegate,UITableViewDataSource,UIViewControllerPreviewingDelegate,UISearchBarDelegate>
...
@implementation LPShortFreightViewController
...
- (void)viewDidLoad {
    ...
    [_tableView registerNib:[UINib nibWithNibName:@"LPTGMYTableViewCell" bundle:nil] forCellReuseIdentifier:@"LPTGCell"];
//    filterController
    __weak LPShortFreightViewController *  weekSelf= self;
    LPShortFreightTitleViewController * filterController=[[LPShortFreightTitleViewController alloc]initWithArea:_SelectAreaData params:self.params Block:^{
...
-(NSMutableDictionary *)params
{
    if (_params==nil) {
        ...
        self.paraname=@"G_ CARLIST";
        self.addr=@"/appcar/get_AllcarList.do";
        ...
        params[@"ADDRESS_TYPE"] =self.SelectAreaData.AreaType;
        switch (a) {
            case 1:
                params[@"ADDRESS_ID"]=self.SelectAreaData.PROVINCE_ID;
                break;
            case 2:
                params[@"ADDRESS_ID"]=self.SelectAreaData.CITY_ID;
                break;
            case 3:
                params[@"ADDRESS_ID"]=self.SelectAreaData.County_ID;
                break;
            case 4:
                params[@"ADDRESS_ID"]=self.SelectAreaData.TOWN_ID;
                break;
            case 5:
                params[@"ADDRESS_ID"]=self.SelectAreaData.VILLAGE_ID;
                break;
        }
    ...
    [LPHttpTool postWithURL:[LPAppInterface GetURLWithInterfaceAddr:_addr] params:_params view:_tableView success:^(id json)
     {
         ...
         if(1==[result intValue])
         {
             NSArray * positionlist =[json objectForKey:@"carlist"];
             [ Global showNoDataImage:_tableView withResultsArray:positionlist];
             if (positionlist.count)
             {

                 _data=[LPTGModel dataWithDic:json];
...
//    LPTGCell
    LPTGModel *model = _data[indexPath.row];
    LPTGMYTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"LPTGCell"];
    [cell.carimage setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",IMAGEPATH,model.photo1]] placeholderImage:[UIImage imageNamed:@""]];
    cell.manName.text = model.name;
    cell.addNew.text = model.arrStr;
    cell.details.text = model.carInforStr;
    return cell;
}
...
//    [self.navigationController pushViewController:PositionDetailsTabBar animated:YES];
    ...
    LPTGModel *model1 = _data[indexPath.row];
    LPTGCarDetailsViewController *cardet = [[LPTGCarDetailsViewController alloc]init];
    cardet.model =model1;
    
    [self.navigationController pushViewController:cardet animated:YES];
